...
function renderOrderEditItemRow($products, $productId = 0, $productName = '', $size = '', $color = '', $qty = 1, $unitPrice = '') {
...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        try {
            ...
            $updateColumns = array('customer_name','customer_email','paid_amount','payment_method','payment_status','shipping_status');
            $updateValues = array($customer_name, $customer_email, $paid_amount, $payment_method, $payment_status, $shipping_status);
            if (columnExists($pdo, 'tbl_payment', 'customer_id')) { $updateColumns[] = 'customer_id'; $updateValues[] = (int)$selected_customer_id; }
            if (columnExists($pdo, 'tbl_payment', 'customer_phone')) { $updateColumns[] = 'customer_phone'; $updateValues[] = $customer_phone; }
            if (columnExists($pdo, 'tbl_payment', 'subtotal')) { $updateColumns[] = 'subtotal'; $updateValues[] = $subtotal; }
            if (columnExists($pdo, 'tbl_payment', 'discount_type')) { $updateColumns[] = 'discount_type'; $updateValues[] = $discount_type; }
            if (columnExists($pdo, 'tbl_payment', 'discount_value')) { $updateColumns[] = 'discount_value'; $updateValues[] = $discount_value; }
            if (columnExists($pdo, 'tbl_payment', 'discount_amount')) { $updateColumns[] = 'discount_amount'; $updateValues[] = $discountAmount; }
            if (columnExists($pdo, 'tbl_payment', 'vat_percent')) { $updateColumns[] = 'vat_percent'; $updateValues[] = $vat_percent; }
            if (columnExists($pdo, 'tbl_payment', 'vat_amount')) { $updateColumns[] = 'vat_amount'; $updateValues[] = $vatAmount; }
            if (columnExists($pdo, 'tbl_payment', 'grand_total')) { $updateColumns[] = 'grand_total'; $updateValues[] = $grandTotal; }
            if (columnExists($pdo, 'tbl_payment', 'due_amount')) { $updateColumns[] = 'due_amount'; $updateValues[] = $dueAmount; }
            if (columnExists($pdo, 'tbl_payment', 'notes')) { $updateColumns[] = 'notes'; $updateValues[] = $notes; }
            if (columnExists($pdo, 'tbl_payment', 'updated_at')) { $updateColumns[] = 'updated_at'; $updateValues[] = date('Y-m-d H:i:s'); }

            $setClauses = array();
            foreach ($updateColumns as $column) {
                $setClauses[] = '`' . $column . '`=?';
            }
            $statement = $pdo->prepare("UPDATE tbl_payment SET " . implode(', ', $setClauses) . " WHERE id=?");
            $updateValues[] = $_REQUEST['id'];
                ...
                <div class="box box-info">
                    <div class="box-body">
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <textarea class="form-control" name="notes" rows="3"><?php echo htmlspecialchars($notes, ENT_QUOTES); ?></textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tbody id="itemsTableBody">
                                    <?php if (!empty($orderItems)): foreach ($orderItems as $item): ?>
                                        <?php echo renderOrderEditItemRow($products, $item['product_id'], $item['product_name'], $item['size'], $item['color'], $item['quantity'], $item['unit_price']); ?>
                                    <?php endforeach; else: ?>
                                        <?php echo renderOrderEditItemRow($products); ?>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="form-group" style="margin-top: 20px;">
                            <div class="col-sm-12">
                                <button type="submit" class="btn btn-success" name="form1">Update Order</button>
